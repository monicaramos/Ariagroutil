...
Begin VB.Form frmManTelef 
   ...
   Begin VB.CheckBox chkAux 
      ...
      Left            =   8280
      TabIndex        =   17
      Tag             =   "Int.Contable|N|N|||telmovil|intconta|||"
      Top             =   4995
      Visible         =   0   'False
      Width           =   210
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   5
      Tag             =   "Total Factura|N|N|||telmovil|totalfac|##,###,#0.00||"
      Top             =   4950
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   4
      Tag             =   "Cuota Iva|N|N|||telmovil|cuotaiva|##,###,#0.00||"
      Top             =   4950
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   3
      Tag             =   "Base Imponible|N|N|||telmovil|baseimpo|##,###,#0.00||"
      Top             =   4950
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   2
      Tag             =   "Fecha de Factura|F|N|||telmovil|fecfactu|dd/mm/yyyy|S|"
      Top             =   4950
      Width           =   1395
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   10
      TabIndex        =   6
      Tag             =   "Cuenta Contable|T|S|||telmovil|codmacta|||"
      Top             =   4950
      Width           =   1395
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   1
      Tag             =   "Numero de Factura|N|N|0|9999999|telmovil|numfactu|0000000|S|"
      Top             =   4920
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   3
      TabIndex        =   0
      Tag             =   "N.Serie|T|N|||telmovil|numserie||S|"
      Top             =   4950
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManTelef.frx":000C
      Height          =   4410
      Left            =   120
      TabIndex        =   11
      Top             =   540
...
'    If Not SepuedeBorrar Then Exit Sub
    ...
    If MsgBox(SQL, vbQuestion + vbYesNo) = vbYes Then
        ...
        SQL = "Delete from telmovil where numserie=" & DBSet(adodc1.Recordset!numserie, "T")
 ...
 TerminaBloquear
    ...
    Select Case Index
        Case 0 'Cuentas Contables (de contabilidad)
            If vParamAplic.NumeroContaTel = 0 Then Exit Sub
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    If vParamAplic.NumeroContaTel <> 0 Then
        Me.Width = 12985
        Me.cmdCancelar.Left = 10410
        Me.cmdAceptar.Left = 9150
        Me.cmdRegresar.Left = 9090
    Else
    ' no hay conexion a la contabilidad
        Me.Width = 9895
        Me.cmdCancelar.Left = 7440
        Me.cmdAceptar.Left = 6180
        Me.cmdRegresar.Left = 6120
    End If
    
    Me.Height = 6705
    
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT telmovil.numserie, telmovil.numfactu, telmovil.fecfactu, "
    CadenaConsulta = CadenaConsulta & "telmovil.baseimpo, telmovil.cuotaiva, telmovil.totalfac, telmovil.intconta, "
    CadenaConsulta = CadenaConsulta & " IF(intconta=1,'*','') as dintconta,telmovil.codmacta "

    ' ### [Monica] 12/09/2006
    ' en caso de haber contabilidad muestro la descripcion de la cuenta
    If vParamAplic.NumeroContaTel <> 0 Then
        CadenaConsulta = CadenaConsulta & ", conta" & vParamAplic.NumeroContaTel & ".cuentas.nommacta "
        CadenaConsulta = CadenaConsulta & "from telmovil left join conta" & DBSet(vParamAplic.NumeroContaTel, "N")
        CadenaConsulta = CadenaConsulta & ".cuentas on (telmovil.`codmacta` = conta" & DBSet(vParamAplic.NumeroContaTel, "N") & ".cuentas.`codmacta`)"
    Else
    ' no hay contabilidad
        CadenaConsulta = CadenaConsulta & "FROM telmovil"
    End If
...
'    adodc1.ConnectionString = Conn
    ...
    End If
    '********************* canviar el ORDER BY *********************++
    SQL = SQL & " ORDER BY telmovil.numserie, telmovil.numfactu"
    '**************************************************************++
    
    CargaGridGnral Me.DataGrid1, Me.adodc1, SQL, PrimeraVez
    
    ' *******************canviar els noms i si fa falta la cantitat********************
    tots = "S|txtAux(0)|T|Ser.|500|;S|txtAux(1)|T|Factura|1000|;"
    tots = tots & "S|txtAux(3)|T|Fecha|1000|;"
    tots = tots & "S|txtAux(4)|T|Base Impon.|1500|;"
...
'            txtAux(Index).Text = UCase(txtAux(Index).Text)
        ...
        Case 2 'cuenta contable
            ...
            txtAux2(Index).Text = PonerNombreCuenta(txtAux(Index), Modo, , cContaTel)
        Case 4, 5, 6
...
'         If ExisteCP(txtAux(0)) Then b = False
        SQL = ""
        SQL = DevuelveDesdeBDNew(cPTours, "telmovil", "numserie", "numserie", txtAux(0).Text, "T", , "numfactu", txtAux(1).Text, "N", "fecfactu", txtAux(3).Text, "F")
        If SQL <> "" Then
