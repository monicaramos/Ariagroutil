...
Begin VB.Form frmManVariedad 
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   10
      TabIndex        =   2
      Tag             =   "Cuenta Contable|T|S|||variedad|codmacta|||"
      Top             =   4950
      Width           =   1395
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   25
      TabIndex        =   1
      Tag             =   "Nombre Variedad|T|N|||variedad|nomvarie||S|"
      Top             =   4950
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   6
      TabIndex        =   0
      Tag             =   "Variedad|N|N|||variedad|codvarie|000000|S|"
      Top             =   4950
      Width           =   800
   End
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManVariedad.frx":000C
      Height          =   4410
      Left            =   120
      TabIndex        =   7
      Top             =   540
...
'    If Not SepuedeBorrar Then Exit Sub
    ...
    If MsgBox(SQl, vbQuestion + vbYesNo) = vbYes Then
        ...
        SQl = "Delete from variedad where codvarie=" & adodc1.Recordset!codvarie
 ...
 TerminaBloquear
    ...
    Select Case Index
        Case 0 'Cuentas Contables (de contabilidad)
            ...
            frmCtas.CadBusqueda = vEmpresaFacSoc.DigGrupoGto
...
'    chkVistaPrevia.Value = CheckValueLeer(Name)
    ...
    If vParamAplic.NumeroContaFacSoc <> 0 Then
        Me.Width = 8960
        Me.cmdCancelar.Left = 7530
        Me.cmdAceptar.Left = 6360
        Me.cmdRegresar.Left = 7530
    Else
    ' no hay conexion a la contabilidad
        Me.Width = 5760
        Me.cmdCancelar.Left = 4330
        Me.cmdAceptar.Left = 3160
        Me.cmdRegresar.Left = 4330
    End If
    
    Me.Height = 6705
    
    '****************** canviar la consulta *********************************+
    CadenaConsulta = "SELECT variedad.codvarie, variedad.nomvarie, variedad.codmacta "

    ' ### [Monica] 12/09/2006
    ' en caso de haber contabilidad muestro la descripcion de la cuenta
    If vParamAplic.NumeroContaFacSoc <> 0 Then
        CadenaConsulta = CadenaConsulta & ", conta" & vParamAplic.NumeroContaFacSoc & ".cuentas.nommacta "
        CadenaConsulta = CadenaConsulta & "from variedad left join conta" & DBSet(vParamAplic.NumeroContaFacSoc, "N")
        CadenaConsulta = CadenaConsulta & ".cuentas on (variedad.`codmacta` = conta" & DBSet(vParamAplic.NumeroContaFacSoc, "N") & ".cuentas.`codmacta`)"
    Else
    ' no hay contabilidad
        CadenaConsulta = CadenaConsulta & "FROM variedad"
    End If
...
'    adodc1.ConnectionString = Conn
    ...
    End If
    '********************* canviar el ORDER BY *********************++
    SQl = SQl & " ORDER BY variedad.codvarie"
    '**************************************************************++
    
    CargaGridGnral Me.DataGrid1, Me.adodc1, SQl, PrimeraVez
    
    ' *******************canviar els noms i si fa falta la cantitat********************
    tots = "S|txtAux(0)|T|Código|900|;S|txtAux(1)|T|Descripción|2600|;"
...
Private Sub txtAux_LostFocus(Index As Integer)
    ...
    Select Case Index
        ...
        Case 2 'cuenta contable
            ...
            If EsCuentaCorrecta(txtAux(Index)) Then
                txtAux2(Index).Text = PonerNombreCuenta(txtAux(Index), Modo, , cContaFacSoc)
            Else
                MsgBox "La Cuenta Contable no es del Grupo de Gastos. Reintroduzca.", vbExclamation
                txtAux2(Index).Text = ""
            End If
    End Select
...
Private Function EsCuentaCorrecta(Cta As String) As Boolean

    EsCuentaCorrecta = (Mid(Cta, 1, 1) = vEmpresaFacSoc.DigGrupoGto)
