...
Begin VB.Form frmManFactCV 
   ...
   Caption         =   "Facturas Varias y de Coarval"
   ClientHeight    =   6825
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   14115
   Icon            =   "frmManFactCV.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   6825
   ...
   Begin VB.ComboBox Combo1 
      ...
      Height          =   315
      Index           =   0
      ItemData        =   "frmManFactCV.frx":000C
      Left            =   -60
      List            =   "frmManFactCV.frx":000E
      Style           =   2  'Dropdown List
      TabIndex        =   0
      Tag             =   "Tipo Factura|N|N|0|2|cvfacturas|tipofactu|||"
      Top             =   4890
      Width           =   735
   End
   Begin VB.CheckBox chkAux 
      ...
      Left            =   10200
      TabIndex        =   10
      Tag             =   "Int.Contable|N|N|||cvfacturas|intconta|||"
      Top             =   4830
      Visible         =   0   'False
      Width           =   210
   End
   ...
   Begin VB.TextBox txtAux 
      ...
      MaxLength       =   30
      TabIndex        =   2
      Tag             =   "Numero de Factura|N|N|0|9999999|cvfacturas|numfactu|0000000|S|"
      Top             =   4920
      Width           =   1395
   End
   Begin VB.TextBox txtAux 
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmManFactCV.frx":0010
      Height          =   4410
      Left            =   120
      TabIndex        =   17
      Top             =   540
   ...
   End
End
Attribute VB_Name = "frmManFactCV"
...
Dim b As Boolean
    ...
    If vParamAplic.NumeroContaCV <> 0 Then
        ...
        btnBuscar(0).Enabled = Not b
        btnBuscar(1).Enabled = Not b
    End If
    btnBuscar(2).visible = Not b
    btnBuscar(2).Enabled = Not b
    ...
    BloquearChk chkAux(0), (Modo = 4) Or (Modo = 3)
End Sub
...
Private Sub BotonModificar()
    ...
    txtAux(10).Text = DataGrid1.Columns(6).Text
    txtAux(9).Text = DataGrid1.Columns(7).Text
    
    txtAux(4).Text = DataGrid1.Columns(8).Text
    txtAux(5).Text = DataGrid1.Columns(9).Text
    txtAux(6).Text = DataGrid1.Columns(10).Text
    txtAux(2).Text = DataGrid1.Columns(13).Text
    txtAux(7).Text = DataGrid1.Columns(14).Text
    i = adodc1.Recordset!TipoFactu
    ' *****************************************************
    ...
    ' ### [Monica] 12/09/2006
'    If vParamAplic.NumeroContaCV <> 0 Then
'        txtAux2(2).Text = DataGrid1.Columns(11).Text
'    Else
'
'    End If
...
'        If AbrirConexionContaCV(vParamAplic.UsuarioContaCV, vParamAplic.PasswordContaCV) Then
'            Set vEmpresaCV = New CempresaCV
'            If vEmpresaCV.LeerNiveles Then
                txtAux2(2) = DevuelveDesdeBDNew(cContaCV, "cuentas", "nommacta", "codmacta", DBLet(Me.adodc1.Recordset!CodmactaSoc, "T"), "T")
                txtAux2(7) = DevuelveDesdeBDNew(cContaCV, "cuentas", "nommacta", "codmacta", DBLet(Me.adodc1.Recordset!CodmactaVta, "T"), "T")
'            End If
'            Set vEmpresaCV = Nothing
'            CerrarConexionContaCV
'        End If
...
'    If vParamAplic.NumeroContaCV <> 0 Then
'        Me.Width = 14285
'        Me.cmdCancelar.Left = 12720
'        Me.cmdAceptar.Left = 11550
'        Me.cmdRegresar.Left = 12720
'    Else
'    ' no hay conexion a la contabilidad
'        Me.Width = 11195
'        Me.cmdCancelar.Left = 8650
'        Me.cmdAceptar.Left = 7480
'        Me.cmdRegresar.Left = 8650
'    End If
...
'    Me.Height = 6705
    ...
    CadenaConsulta = CadenaConsulta & "cvfacturas.baseimpo, cvfacturas.cuotaiva,cvfacturas.totalfac, cvfacturas.intconta, IF(intconta=1,'*','') as dintconta , "
    CadenaConsulta = CadenaConsulta & " cvfacturas.codmactasoc, cvfacturas.codmactavta "
...
'    If vParamAplic.NumeroContaCV <> 0 Then
'        CadenaConsulta = CadenaConsulta & ", conta" & vParamAplic.NumeroContaCV & ".cuentas.nommacta "
'        CadenaConsulta = CadenaConsulta & "from cvfacturas left join conta" & DBSet(vParamAplic.NumeroContaCV, "N")
'        CadenaConsulta = CadenaConsulta & ".cuentas on (cvfacturas.`codmactasoc` = conta" & DBSet(vParamAplic.NumeroContaCV, "N") & ".cuentas.`codmacta`)"
'    Else
'    ' no hay contabilidad
'        CadenaConsulta = CadenaConsulta & "FROM cvfacturas"
'    End If
...
'    adodc1.ConnectionString = Conn
    ...
    End If
    '********************* canviar el ORDER BY *********************++
    Sql = Sql & " ORDER BY cvfacturas.letraser, cvfacturas.numfactu"
    '**************************************************************++
    ...
    tots = tots & "S|txtAux(0)|T|Ser.|500|;S|txtAux(1)|T|Factura|1050|;"
    tots = tots & "S|txtAux(3)|T|Fecha|1200|;"
    tots = tots & "S|btnBuscar(2)|B|||;"
    tots = tots & "S|txtAux(8)|T|Nif Socio|1200|;"
    tots = tots & "S|txtAux(10)|T|T.Iva|600|;"
    tots = tots & "S|txtAux(9)|T|%Iva|600|;"
    tots = tots & "S|txtAux(4)|T|Base Imponible|1350|;"
    tots = tots & "S|txtAux(5)|T|Cuota Iva|1350|;"
    tots = tots & "S|txtAux(6)|T|Total Factura|1350|;"
    tots = tots & "N||||0|;S|chkAux(0)|CB|IC|360|;"
    tots = tots & "S|txtAux(2)|T|Cta.Contable|1200|;"
    tots = tots & "S|btnBuscar(0)|B|||;"
    tots = tots & "S|txtAux(7)|T|Cta.Cble Vtas|1200|;"
...
'    If vParamAplic.NumeroContaCV <> 0 Then
'        tots = tots & "S|btnBuscar(0)|B|||;S|txtAux2(2)|T|Nombre de Cuenta|3200|;"
'    End If
...
'         If ExisteCP(txtAux(0)) Then b = False
        Sql = ""
        Sql = DevuelveDesdeBDNew(cPTours, "cvfacturas", "letraser", "letraser", txtAux(0).Text, "T", , "numfactu", txtAux(1).Text, "N", "fecfactu", txtAux(3).Text, "F")
        If Sql <> "" Then
